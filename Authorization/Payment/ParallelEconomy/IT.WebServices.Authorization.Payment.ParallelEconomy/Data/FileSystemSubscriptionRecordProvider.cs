...
#pragma warning restore CS1998
        ...
        public Task<ParallelEconomySubscriptionRecord?> GetById(Guid userId, Guid subId)
        {
            var fi = GetDataFilePath(userId, subId);
        ...
        private async IAsyncEnumerable<ParallelEconomySubscriptionRecord> ReadHistoryFromFile(FileInfo fi)
        {
            if (!fi.Exists)
                yield break;
            ...
            await foreach(var line in File.ReadLinesAsync(fi.FullName))
            {
                ...
                yield return ParallelEconomySubscriptionRecord.Parser.ParseFrom(Convert.FromBase64String(line));
            }
        }

        private async Task<ParallelEconomySubscriptionRecord?> ReadLastOfFile(FileInfo fi)
        {
            if (!fi.Exists)
                return null;
