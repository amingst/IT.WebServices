...
namespace IT.WebServices.Authorization.Payment.Fortis.Data
{
    internal class FileSystemPaymentRecordProvider : IPaymentRecordProvider
    {
        ...
        public Task<FortisPaymentRecord?> GetById(Guid userId, Guid subId, Guid paymentId)
        {
            var fi = GetDataFilePath(userId, subId, paymentId);
            return ReadLastOfFile(fi);
        }

        public async Task Save(FortisPaymentRecord rec)
        {
        ...
        private async IAsyncEnumerable<FortisPaymentRecord> ReadHistoryFromFile(FileInfo fi)
        {
            if (!fi.Exists)
                yield break;
            ...
            await foreach (var line in File.ReadLinesAsync(fi.FullName))
            {
                ...
                yield return FortisPaymentRecord.Parser.ParseFrom(Convert.FromBase64String(line));
            }
        }

        private async Task<FortisPaymentRecord?> ReadLastOfFile(FileInfo fi)
        {
            if (!fi.Exists)
                return null;
            ...
            return FortisPaymentRecord.Parser.ParseFrom(Convert.FromBase64String(last));
        }
    }
}
