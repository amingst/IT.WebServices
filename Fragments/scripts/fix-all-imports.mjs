...
function fixAllImports(file) {
  if (!fs.existsSync(file)) return;
  
  let content = fs.readFileSync(file, 'utf8');
  let modified = false;

  // Fix all import/export patterns that need .js extensions or /index.js for directories
  const patterns = [
    /from\s+['"](\.[^'"]*?)['"];?/g,
    /export\s+\*\s+from\s+['"](\.[^'"]*?)['"];?/g,
...
console.log('Fixing all ESM imports...');
walk(esmRoot);
console.log('All ESM import fixing complete');