// @generated by protoc-gen-connect-es v0.13.0 with parameter "target=ts,import_extension=none"
// @generated from file Protos/IT/WebServices/Fragments/Authorization/Payment/BackupInterface.proto (package IT.WebServices.Fragments.Authorization.Payment, syntax proto3)
/* eslint-disable */
// @ts-nocheck

import { BackupAllDataRequest, BackupAllDataResponse, RestoreAllDataRequest, RestoreAllDataResponse } from "./BackupInterface_pb";
import { MethodKind } from "@bufbuild/protobuf";

/**
 * Service for Payment backup fragment interface
 *
 * @generated from service IT.WebServices.Fragments.Authorization.Payment.BackupInterface
 */
export const BackupInterface = {
  typeName: "IT.WebServices.Fragments.Authorization.Payment.BackupInterface",
  methods: {
    /**
     * Export a list of all data.
     *
     * @generated from rpc IT.WebServices.Fragments.Authorization.Payment.BackupInterface.BackupAllData
     */
    backupAllData: {
      name: "BackupAllData",
      I: BackupAllDataRequest,
      O: BackupAllDataResponse,
      kind: MethodKind.ServerStreaming,
    },
    /**
     * Restore a list of all data.
     *
     * @generated from rpc IT.WebServices.Fragments.Authorization.Payment.BackupInterface.RestoreAllData
     */
    restoreAllData: {
      name: "RestoreAllData",
      I: RestoreAllDataRequest,
      O: RestoreAllDataResponse,
      kind: MethodKind.ClientStreaming,
    },
  }
} as const;

